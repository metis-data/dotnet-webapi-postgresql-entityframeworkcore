$location = (get-location).path
$Key = "${location}\ssh_tunnel\tunnel_rsa"
Icacls $Key /c /t /Inheritance:d
TakeOwn /F $Key
Icacls $Key /c /t /Grant:r ${env:UserName}:F
Icacls $Key /c /t /Remove:g Administrator "Authenticated Users" BUILTIN\Administrators BUILTIN Everyone System Users
Icacls $Key

docker build -t dotnet_webapi_postgresql_entityframeworkcore_ssh ssh_tunnel

docker start dotnet_webapi_postgresql_entityframeworkcore_ssh

if($LASTEXITCODE -ne 0){
docker run -d `
  --name=dotnet_webapi_postgresql_entityframeworkcore_ssh `
  -e TZ=Etc/UTC `
  -e "PUBLIC_KEY=ssh-rsa " `
  -p 127.0.0.1:58222:2222 `
  -e USER_NAME=tunnel `
  --restart unless-stopped `
  dotnet_webapi_postgresql_entityframeworkcore_ssh
}

sleep 5

ssh-keygen -R '[localhost]:58222'
$replacedLocation = $location.replace("\", "/")
(Get-WmiObject win32_process -filter "Name='ssh.exe' AND CommandLine LIKE '%${replacedLocation}/ssh_tunnel/tunnel_rsa tunnel@localhost -p 58222%'").Terminate()
$sshStartBlock = { param([string]$pwd) ssh -i "$pwd/ssh_tunnel/tunnel_rsa" tunnel@localhost -p 58222 -4 -o StrictHostKeyChecking=no -R 5432:127.0.0.1:5432 -R 4318:127.0.0.1:4318 -L 3000:127.0.0.1:3000 -fN }
start-job -ScriptBlock $sshStartBlock -ArgumentList $replacedLocation

docker build -t dotnet_webapi_postgresql_entityframeworkcore .

docker start dotnet_webapi_postgresql_entityframeworkcore
if($LASTEXITCODE -ne 0){
docker run `
  --name=dotnet_webapi_postgresql_entityframeworkcore `
  --network 'container:dotnet_webapi_postgresql_entityframeworkcore_ssh' `
  dotnet_webapi_postgresql_entityframeworkcore
}
